...
export function prepare_systems(scene: Scene, character: Character_capsule) {
	...
	const gui_control = new GuiControl(ui)
	const heating_system = new HeatingSystem()

	scene_items.on_item_added(
		gui_control.create_guis,
		heating_system.add_heat_source,
		heating_system.add_cookable
	)
